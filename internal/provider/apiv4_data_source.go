	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"flow_execution": schema.SingleNestedAttribute{
				...
				Attributes: map[string]schema.Attribute{
					...
					"mode": schema.StringAttribute{
						Computed:    true,
						Description: `API's flow mode. ` + "`" + `DEFAULT` + "`" + ` delegate to Flow's selectors where ` + "`" + `BEST_MATCH` + "`" + ` will use the path to select the flow.`,
					},
				},
			...
			"state": schema.StringAttribute{
				Computed:    true,
				Description: `The desired state of the API. Switching states between STOPPED and STARTED will effectively deploy the API and vice-versa.`,
			},
			"tags": schema.ListAttribute{
				Computed:    true,
				ElementType: types.StringType,
